...
def get_data_path(filename):
    ...
    Returns Text: Full path for the filename.
    """
...
def load_config(filename=None):
    ...
    Returns dict: Dictionary loaded from config file.
    """
    args = ['', '']
    if filename:
...
def parse_ccpem_collection(collection):
    ...
    Returns List[Dict]: Text and segments parts of the collection.
    """
    return [[{
        'text': utterance['text'],
...
def get_conversations(collection):
    ...
    Returns List[List]: List of utterances in a list of conversations
    """
...
def get_segments(collection, annotationTypes=None):
    ...
    Returns List(dict): Collection with annotations equal to annotation types.
    """
    segments = []
    for conversation in collection:
...
def get_annotation_types(collection):
    ...
    Returns Dict: All annotation types.
    """
...
def get_slot_annotator(config):
    ...
    Returns SlotAnnotator: Returns a SlotAnnotator instance.
    """
    agent = Agent(config)
    agent.initialize()
...
def annotate_slot_utterance(annotator, slot, utterance):
    ...
    Returns List[Dict]: List of semantic annotations for utterance.
    """
...
def annotate_conversation_for_slot(annotator, slot, conversation, i=None):
    ...
    Returns List[List[Dict]]: List of semantic annotations for conversation.
    """
    if i:
...
def remove_duplicates(annotations):
    ...
    Returns List: List of annotations without duplicates.
    """
    return [
...
def get_matches(annotations, true_annotations):
    ...
    Returns int: Number of matching annotations.
    """
    return sum(
...
def evaluate(conversations, truth_segments):
    ...
    Returns (float, float, float): Precision, Recall and f1 scores
    """
...
def get_annotations(slots, entity_types, force=False):
    ...
    Returns Dict: Dictionary with annotatins and durations for each slot.
    """
...
def get_errors(annotations, truth_segments, conversations):
    ...
    Returns List[Dict]: Dictionary containing the original utterance, system
